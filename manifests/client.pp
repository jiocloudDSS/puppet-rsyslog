...
class rsyslog::client (
  ...
  $custom_params             = undef,
 # $server                    = 'log',
  $server                    = '10.135.38.155', #centralized logging server is bstrap of idc1 in staging for the time being, after adding virtual IP in HAMode, mon6 is the backup right now 
  $port                      = '514',
